...
(function() {{
  ...
  function safeOverlay(url, bnds, opts) {{
    var ov = L.imageOverlay(url, bnds, opts).addTo(map);
    var el = ov.getElement();
    if (el) {{ el.onerror = function() {{ map.removeLayer(ov); }}; }}
    return ov;
  }}
  ...
  window.addEventListener('message', function(e) {{
    ...
    if (e.data.type === 'setLayer' && costLayer) {{
      map.removeLayer(costLayer);
      costLayer = safeOverlay(
