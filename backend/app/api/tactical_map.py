...
# ═══════════════════════════════════════════════════════════
# 입력 지도 클릭/마커 in-memory 상태 (CommanderInputPage 전용)
# 단일 상태 관리: departure + targets 통합, marker ID 충돌 없음
# ═══════════════════════════════════════════════════════════
from pydantic import BaseModel as _PydBase


_im_click_seq: int = 0
_im_pending: dict | None = None
_im_marker_seq: int = 0
_im_markers: list = []


class _ImClickBody(_PydBase):
    unit: int = 0
    lat: float
    lng: float
...
class _ImMarkersBody(_PydBase):
    markers: list[_ImMarkerItem]


@router.post("/map/tactical/im/click", include_in_schema=False)
async def im_record_click(payload: _ImClickBody):
    """지도 우클릭 좌표 기록 (kind: arrival | departure)."""
    global _im_click_seq, _im_pending
    _im_click_seq += 1
    _im_pending = {
        "seq": _im_click_seq,
        "unit": payload.unit,
    ...
    return {"ok": True, "seq": _im_click_seq}


@router.get("/map/tactical/im/pending-click", include_in_schema=False)
async def im_get_pending_click():
    """미처리 클릭 조회 (Python 폴링용)."""
    if _im_pending is None:
        return {"seq": 0, "unit": None, "lat": None, "lng": None, "kind": None}
    return _im_pending


@router.get("/map/tactical/im/markers", include_in_schema=False)
async def im_get_markers():
    """현재 마커 목록 조회."""
    return {"seq": _im_marker_seq, "markers": _im_markers}


@router.post("/map/tactical/im/markers", include_in_schema=False)
async def im_set_markers(payload: _ImMarkersBody):
    """마커 업데이트 (Solara → 지도 전파)."""
    global _im_marker_seq, _im_markers
    _im_marker_seq += 1
    _im_markers = [
        {"id": m.id, "lat": m.lat, "lng": m.lng, "kind": m.kind}
        for m in payload.markers
    ]
    return {"ok": True, "seq": _im_marker_seq}
